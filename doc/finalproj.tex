...
Then a tracking mechanism based on a simple dynamical model and correlation is used to pruned out many noisy descriptors. The surviving
...
\item A tracking mechanism based on a simple dynamical model and correlation is used to pruned out many noisy descriptors.
...
between two images are defined to be the normalized scalar product between their respective tf-idf statistics. This relevance measure is
...
For this task, the choice of detector is Difference of Gaussian (DoG) and the descriptor is SIFT. These are standard, state-of-the-art choice for many
computer vision systems and thus readily-available implementations are available online. This project used the VLFeat \cite{vedaldi08vlfeat} toolbox
in particular.
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.7\linewidth]{ANRR}
